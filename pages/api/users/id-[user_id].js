...
apiRoute.get(async (req, res) => {
  ...
  if (!user_id) {
    res.status(400).json({
      message: "Veuillez fournir un identifiant d'utilisateur.",
    });
  } else {
    let sql =
      "SELECT user_id, username, display_name, profile_picture_url, user_description, created_at, updated_at FROM users WHERE user_id = $1";
    let params = [user_id];
    ...
    limit(() => {
      connection.query(sql, params, (error, results) => {
        ...
        } else if (results.rowCount === 0 && user_id) {
          res.status(404).json({
